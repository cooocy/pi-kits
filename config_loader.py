...
# Filename of your configurations.
config_filename = '/config_private.yaml'


def __load_config():
    current_path = kits.current_path()
    f = open(current_path + config_filename)
    y = yaml.safe_load(f)
    f.close()
    return y
...
reporter__ = Reporter(__configurations['reporter']['base_url'],
                      ...
                      __configurations['reporter']['lan_status_report_uri'])
